...
import { FaUser, FaEnvelope, FaLock, FaUserShield, FaArrowRight, FaSignInAlt, FaUserPlus } from "react-icons/fa";
...
const Auth = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (isLogin) {
      ...
      dispatch(signUp(formData.name, formData.email, formData.password, formData.role, "Active", navigate));
    }
  };
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center bg-slate-950 overflow-hidden text-slate-100 font-sans">
      {/* Dynamic Animated Glowing Background Blobs */}
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-md mx-4 p-8 rounded-3xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl shadow-[0_0_50px_-12px_rgba(99,102,241,0.25)]"
      >
        {/* Toggle Controls */}
        <div className="flex bg-slate-950/80 p-1.5 rounded-2xl border border-slate-800/50 mb-8 relative">
          <div className="grid grid-cols-2 w-full relative z-10">
            <button
              ...
              className={`flex items-center justify-center gap-2 py-2.5 rounded-xl font-medium text-sm transition-all duration-300 ${
                isLogin ? "text-indigo-400 font-semibold" : "text-slate-400 hover:text-slate-200"
              }`}
            >
          ...
          <motion.div
            ...
            className="absolute top-1.5 bottom-1.5 left-1.5 w-[calc(50%-6px)] bg-slate-900 border border-indigo-500/30 rounded-xl"
            style={{
              x: isLogin ? 0 : "100%",
            }}
          />
              ...
              {!isLogin && (
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none transition-all placeholder:text-slate-600 hover:border-slate-700"
                  />
                </div>
              )}
              ...
              {!isLogin && (
                <div className="relative group">
                  ...
                  <select
                    ...
                    value={formData.role}
                    onChange={handleChange}
                    className="w-full pl-11 pr-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none transition-all hover:border-slate-700 text-slate-300"
                  >
                    <option value="User" className="bg-slate-950 text-slate-100">Standard User</option>
